...
public class TenantDatabaseMetaDataPorxy implements DatabaseMetaData {
    private final TenantConnectionInfo connectionInfo;

    private final DatabaseMetaData delegate;

    public TenantDatabaseMetaDataPorxy(DatabaseMetaData metaData, TenantConnectionInfo connectionInfo) {
    ...
    public String getURL() throws SQLException {
        ...
        if (qPos != -1) {
            return getURL(jdbcUrl, "&", connectionInfo);
        }
        return getURL(jdbcUrl, "?", connectionInfo);
    }

    private String getURL(String url, String prefix, TenantConnectionInfo ci) {

        return url + prefix + "tenant=" + ci.getTenant() + "&ds_key=" + ci.getDatasource() + "&unique_code=" + ci.getUniqueCode().get();
    }
